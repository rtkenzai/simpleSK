...
<td><b>TINDAKAN</b></td>	
 ...
 <?php
  $no=1;
  $data1=mysqli_query($con," SELECT p.nama,p.idpeserta, m.idpeserta,
SUM(m.markah) AS Jum,  
m.markah FROM peserta AS p 
INNER JOIN markah AS m 
ON p.idpeserta=m.idpeserta 
GROUP BY m.idpeserta 
...
<input type="text" name="id[]" value="<?php echo $info1['idpeserta']; ?>" hidden>
<input type="text" name="jum[]" value="<?php echo $info1['Jum']; ?>" hidden>
<input type="text" name="tempat[]" value="<?php echo $hadiah; ?>" hidden>
<!-- TAMAT DI SINI -->
</td>
<td>
<a href="pemenang_laporan.php?id=
<?php echo $info1['idpeserta'];?>&rank=<?php echo $hadiah;?>
